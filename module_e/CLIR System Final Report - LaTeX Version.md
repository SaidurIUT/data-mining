...
> **Note:** This report describes a bilingual English-Bangla CLIR system. For LaTeX compatibility, all Bangla text examples have been replaced with English translations or generic placeholders. The actual system processes native Bangla script as demonstrated in the accompanying CSV result files and code repository.
...
```json
{
  "title": "Election Commission Dialogue Begins",
  "body": "The Election Commission today started dialogue with various political parties...",
  "source": "prothom-alo",
...
```python
ENTITY_MAPPINGS = {
    'BNP': ['BNP Bangla', 'Bangladesh Nationalist Party'],
    'Awami League': ['Awami League Bangla', 'AL'],
    'Dhaka': ['Dhaka Bangla', 'Dhaka city'],
    'election': ['election Bangla', 'vote', 'electoral'],
}

def expand_query(query):
    expanded = [query]
    for eng, variants in ENTITY_MAPPINGS.items():
...
Our system handles cross-script named entities through the semantic embedding model, which learns that "Dhaka" and its Bangla equivalent refer to the same entity. This is more robust than dictionary-based mapping as it handles unseen entities through learned representations.

---

### 3.8 Retrieval Models

#### 3.8.1 BM25 (Lexical Retrieval)
...
#### 3.8.2 Semantic Search (Neural Retrieval)
...
#### 3.8.3 Fuzzy Search (Transliteration Handling)
...
### 3.9 Ranking and Scoring Approach

#### 3.9.1 Score Normalization
...
| "BNP" (party name) | Same-language | Bangla |
| "Election Commission" | Same-language | Bangla |
| "Dhaka University" | Same-language | Bangla |
...
**Interpretation:** BM25 completely fails because "election" has no lexical overlap with its Bangla equivalent. Semantic search bridges this gap through multilingual embeddings.
...
**Interpretation:** "BNP party" contains English text that doesn't appear in Bangla documents, so BM25 fails. Semantic search understands the cross-lingual relationship.
...
| "BNP" | Same-language | 1.00 | 0.90 | 1.00 | 31 |
| "Election Commission" | Same-language | 1.00 | 1.00 | 1.00 | 41 |
| "Dhaka University" | Same-language | 1.00 | 0.50 | 0.60 | 39 |
...
*Note: Full-color matplotlib visualizations are included in the `visualizations/` directory.*
...
|-----------|---------|-------------|
| Same-language query | "BNP" in Bangla | Direct term matching works perfectly |
| Exact phrase needed | "Election Commission" | Multi-word phrases matched precisely |
| Speed is critical | Real-time autocomplete | 150x faster than semantic |

**Concrete Example:**
```
Query: "BNP" (party name in Bangla)
BM25 Result:  Finds all 31 documents containing "BNP" → P@10 = 1.00
...
| Condition | Example | Explanation |
|-----------|---------|-------------|
| Cross-lingual query | "election" → Bangla docs | Embeddings bridge language gap |
| Conceptual matching | "voting fraud" → related terms | Understands meaning |
| Named entity translation | "Dhaka" → Bangla equivalent | Same entity, different scripts |

**Concrete Example:**
```
Query: "election" (English)
BM25 Result:  0 documents (no English text in Bangla corpus) → P@10 = 0.00
Semantic:     28 documents about elections → P@10 = 1.00
```
...
**Test Case:** Query "prime minister" → Documents about the prime minister in Bangla
...
```
Title: "Prime Minister Sheikh Hasina Gives Speech in Parliament Today"
Semantic Score: 0.38
...
**Test Case:** Query "Dhaka" → Documents containing Dhaka in Bangla script
...
**Why Semantic Works:**
LaBSE was trained on translation pairs, so it learned that "Dhaka" and its Bangla equivalent refer to the same entity through their co-occurrence in parallel corpora.

#### 5.2.3 Cross-Script Ambiguity

**Definition:** Same word has different meanings or spellings across scripts.

**Test Case:** "University" in Bangla has multiple transliterations

| Transliteration | Meaning | Semantic Handles? |
|-----------------|---------|-------------------|
| Transliteration A | University | ✅ |
...
```
Query: "BNP party"
Expected: Documents about BNP (Bangladesh Nationalist Party) in Bangla

BM25 Result: 0 documents (no English text in corpus)
Semantic Result: 21 relevant documents (understands cross-lingual association)
```
...
**Test Case:** "Dhaka city" → Some documents discuss Dhaka without using the word "city"

| Document Type | Contains "Dhaka"? | Contains "city" equivalent? | Retrieved? |
|---------------|-------------------|-----------------------------|------------|
| City news | ✅ | ✅ | ✅ |
...
|-----------|---------|------------------|
| Script variation | "Dhaka" vs Bangla equivalent vs "Dacca" | ✅ Works via embeddings |
| Acronyms | "BNP" vs Bangla BNP | ✅ Works via embeddings |
| Partial names | "Sheikh Hasina" vs Bangla vs "PM Hasina" | ⚠️ Inconsistent |
...
└── module_e/                            # Report (Module E)
    ├── CLIR System Final Report.md     # Original report
    ├── CLIR System Final Report - LaTeX Version.md  # This file
...
|------|----------------|-------|-----------|
| 1 | Election Commission Dialogue Begins | 0.42 | ✅ |
| 2 | BNP Joins Election | 0.39 | ✅ |
| 3 | Voter List Updated | 0.38 | ✅ |
| 4 | Political Situation Discussion | 0.35 | ✅ |
| 5 | Electoral Law Amendment | 0.33 | ✅ |

**Query: "BNP" (Same-language)**

| Rank | Document Title | Score | Relevant? |
|------|----------------|-------|-----------|
| 1 | BNP Secretary General Statement | 0.89 | ✅ |
| 2 | BNP Leaders Rally | 0.85 | ✅ |
| 3 | BNP Political Program | 0.82 | ✅ |
| 4 | BNP-Jamaat Alliance | 0.79 | ✅ |
| 5 | BNP Demand Rejected | 0.76 | ✅ |
